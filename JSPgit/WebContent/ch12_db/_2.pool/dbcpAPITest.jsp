<%-- 1. Java Naming and Directory interface(JNDI)는 Java 애플리케이션이 네트워크에서
		 이름(name)을 이요하여 객체를 찾고 가져올 수 있도록 하는 표준 API입니다.
		 
		 * JNDI를 사용 하면 Java 프로그램이나 JSP 페이지에서 으용 프로그램 내부 또는 외부에서 지정된
		 자원을 참조 할 수 있습니다.
	 ...
	 2. DBCP(Database Connection Pool)
	 	...
	 	웹 프로그램에서는 데이터베이스의 환경설정고 ㅏ연결 관리 등을 따로 XML파일이나
	 	속성 파일을 사용해서 관리하고 이렇게 설정된 정보를 이름을 사용하여 획득하는 방법을 사용합니다.
	 ...
	 3. DataSource 이용 방법
	 	(1) JNDI Server에서 lookup()메소드를 통해 DataSource객체를 얻어옵니다.
	 	(2) DataSource 객체의 getConnection()메소드를 통해서 Connection Pool에서 
	 		Connection을 획득합니다.
...
<%
	...
	try{
		...
		//jdbc/OracleDB를 참조하여 Connection 객체를 얻어 옵니다.
		
		//1. JNDI를 이용하기 위한 객체 생성합니다.
		Context init = new InitialContext();
		
		/*2. JNDI의 모든 이름은 기본적으로 java:comp/env/에 등록되어 있습니다.
			 Jdbc/OracleDB:JNDI 서비스에 접근하기 위한 이름입니다.
			 lookup(): 등록된 naming 서비스로부터 자원을 찾고자할 때 사용하는 메소드
