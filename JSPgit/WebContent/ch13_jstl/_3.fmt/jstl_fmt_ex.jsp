...
<body>
<%-- <fmt:setLocale>다국어 페이지를 사용할 언어를 지정하는 태그입니다.
	value : 어떤 언어를 사용할 지 지정하는 속성입니다.
			ko_KR에서 ko는 한글의 언어 코드이고
			KR은 한국의 국가 코드를 의미합니다.
			value값이 null이면 기본 로케일이 사용됩니다. --%>
...
<%-- <fmt:bundle>
	...
	여기서 국가는 생략 가능합니다. --%>
<%-- 설정한 properties 파일의 리소스 내용을 읽을 때 사용합닌다.
	...
	<%--fmt:message 태그로 읽어 온 리소스 내용에 파라미터를 전달하는 태그
		test.properties에 {0}로 전달됩니다.(setLocale이 en_US일때)
		test._ko.properties에{0}로 전달됩니다.(setLocale이 ko_KR일때)--%>
		<fmt:param value="고길동"/>
	<%--test.properties에 {1}로 전달됩니다.(setLocale이 en_US일 때)
		test_ko.properties에{1}로 전달됩니다.(setLocale이 ko_KR일 때) --%>
		<fmt:param value="전우치"/>
	</fmt:message>
</fmt:bundle>
</body>
